...
function addText(text, x, y, multiplier, group, floor, mid_x, mid_y) {
...
camera_first.rotation.x = 1.5707963267948963;
...
for (var i=0; i<2; i++){
    ...
    for (let j = 0; j < list_of_text[i].length; j++){
        addText(list_of_text[i][j][0], list_of_text[i][j][1], list_of_text[i][j][2], 2, group, i, mid_x, mid_y)
    }
    scene.add(group)
    groups.push(group)
    mid_x = 424
...
function animate() {
    ...
    n -= 74;
    console.log(camera_first.rotation)
    };
    if (n>1){
        camera_first.rotation.x = 1.5707963267948963;
        camera_first.rotation.z = 0;
